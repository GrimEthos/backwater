    ...
    namespace auth
    {
        ...
        struct IAuth
        {
            ...
            virtual Passcode                makePasscode(
                                                ...
                                                HostKey ) = 0;
            //! idInit
            using                           idInitCallback = std::function<void( Result, Timecode )>;
            virtual void                    idInit(
                                                DeviceIP ip,
                                                cpp::Memory email, 
                                                cpp::Memory svc, 
                                                idInitCallback callback ) = 0;
            ...
            virtual void                    auth(
                                                DeviceIP authorizingIp,
                                                AuthToken authorizingToken,
                                                DeviceIP requesterIp,
                                                AuthToken requesterToken,
                                                cpp::Memory svc,
                                                authCallback callback ) = 0;
            //! multiauth
            struct                          AuthFailure { Result reason; AuthToken token; };
            struct                          AuthSuccess { AuthToken token; UserId userId; UserEmail email; };
            using                           multiauthCallback = std::function<void( 
                                                Result result, 
                                                size_t failCount, 
                                                AuthFailure * fail,
                                                size_t successCount, 
                                                AuthSuccess * success )>;
            virtual void                    multiauth(
                                                DeviceIP authorizingIp,
                                                AuthToken authorizingToken,
                                                size_t authTokenCount,
                                                AuthToken * authToken,
                                                cpp::Memory svc,
                                                authCallback callback ) = 0;
        };
